(function () {
  ...
  window.AtlasApplyIcelandContent = ({ places, copy }) => {
    ...
    places.splice(0, places.length, ...content.discoveries.map(item => ({ id: item.id, name: item.locales.en.place, coordinates: item.coordinates, source: sourceUrl(content, item) })));
  ...
  window.AtlasApplyRegionalContent = regions => {
    for (const [id, content] of Object.entries(packages)) {
      ...
      for (const locale of ['en', 'fr', 'sv']) {
        ...
        existing.discoveries = content.discoveries.map(item => { const local = item.locales[locale]; return { place: local.place, coordinates: item.coordinates, clue: local.clue, field: local.fieldNote, evidence: local.evidence.map(card => [card.label, card.text]), question: local.question, options: local.options, correct: local.correct, reveal: local.reveal, source: sourceUrl(content, item) }; });
      }
    }
    document.documentElement.dataset.contentSource = 'canonical-json';
  };
